...
public class AuthenticationControllerTest {
  ...
  void setUp() {
    ...
    validAuthTokens =
        new AuthTokens(
            refreshTokenId, accessToken, userId, "exampleDisplayName", "exampleProfileUrl");
  ...
  void signup_invalidRequestWithMultipleFieldError_returns400() throws Exception {
    // Setup
    SignupRequest invalidPassword = new SignupRequest("handle", "123_?", null, null, null);
  ...
  void logout_invalidRefreshToken_returns401() throws Exception {
    // Setup
    String token = "api-key-value";
    doThrow(new InvalidRefreshTokenException("Invalid refresh token"))
  ...
  void refresh_invalidRefreshToken_returns401() throws Exception {
    // Setup
    String refreshTokenIdRequest = UUID.randomUUID().toString();
    when(authenticationService.refresh(refreshTokenIdRequest))
